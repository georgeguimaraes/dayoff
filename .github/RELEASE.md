...
```
feat: support the omit clause in if-holiday rules

fix: sync date-holidays data 1a2b3c4

chore: upgrade dependencies to latest versions

docs: update installation instructions

refactor: split the table calendars into one module
```
...
BREAKING CHANGE: holidays/3 returns structs instead of maps
```
...
Check the workflow logs at: https://github.com/georgeguimaraes/dayoff/actions/workflows/release-please.yml
...
The `sync` workflow runs daily, checks whether the data folders of the upstream repositories moved, and when the tests still pass commits `fix: sync date-holidays data <sha>` straight to `main`. That commit is what makes release-please cut a patch release with the new data. The push uses the `SYNC_TOKEN` repository secret, a fine-grained personal access token with contents and pull requests write access, because pushes made with the default `GITHUB_TOKEN` don't trigger other workflows and release-please would never see them. When the tests fail, the workflow opens a pull request with the diff and the failing parity cases instead.
